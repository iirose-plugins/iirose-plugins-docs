...
点播消息分为用户发送的点播卡片（用户发送的消息）和触发播放（服务器触发的事件）两种情况。

用户可以点播多个歌曲，每次点播将会触发点播卡片的发送。

等待播放的歌曲将被添加到房间的播放列表下，每当歌曲开始播放的时候，播放事件将被触发。

#### 点播卡片

点播成功的时候 用户会自动触发 **点播卡片** 发送到群里。
...
#### 媒体播放更新
- **触发时机**: 房间的媒体播放列表或当前播放状态发生变化时。
- **对应解码器**: `Music.ts`, `MusicMessage.ts`
...
[00:02.000] 编曲 : ChiliChill
[00:03.000] 制作人 : ChiliChill
...
[00:25.606]嫌弃下雨的天气
[03:08.106]在哪里呢 在哪里呢
[03:11.353]晒干这一片蓝色
