...
在与 IIROSE 服务器进行任何交互之前，客户端必须首先建立一个稳定的 WebSocket 连接。本文档将详细说明如何选择最佳服务器并完成连接。
...
客户端应并行地尝试与上述列表中的所有服务器建立 WebSocket 连接，并测量从发起连接到成功 `open` 事件触发所花费的时间。选择耗时最短（即延迟最低）的服务器作为本次会话的通信地址。

:::tip 提示
如果所有服务器都无法在设定的超时时间（例如5秒）内成功连接，客户端应等待一段时间后（例如5秒）进行重试，直到找到可用的服务器。
:::
