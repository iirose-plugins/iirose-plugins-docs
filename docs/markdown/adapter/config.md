...
**描述**: BOT的用户名<br>`不带[**]的部分`<br>可填写 `[*用户名*]`，适配器会自动剥离并 trim 首尾空格
...
**描述**: BOT的唯一标识<br>`不带[@@]的部分`<br>可填写 `[@uid@]` 或大写形式，适配器会自动剥离并转为小写
...
**描述**: BOT的初始房间地址<br>`不带[__]的部分`<br>可填写 `[_房间ID_]`，适配器会自动剥离并 trim 首尾空格
